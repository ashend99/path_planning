...
class Graph:
    ...
    def get_closest_node(self, xy):
        ...
        for i in range(len(self.nodes_)):
            ...
            # x and y pixel cordinates
            x = xy[0]
            y = xy[1]
            dist = math.sqrt((x-self.nodes_[i].x)**2 + (y-self.nodes_[i].y)**2) # distance between two points

            # check for closest node
            if dist < best_dist:
                best_dist = dist    # shortest distance
                best_index = self.nodes_[i].idx # closest node
    ...
    def find_connected_groups(self):
        ...
        # connecting nodes into groups
        for idx in range(len(groups)):
            # not yet connected to a group
            if groups[idx] == 0:
                # get all the nodes that can traversable
                connected_nodes = graph_search.find_connected_nodes(idx)

                # check if a same node is in multi groups
                disconnected_groups = []
                for connected_idx in connected_nodes:
                    if groups[connected_idx] != 0:
                        if not groups[connected_idx] in disconnected_groups:
                            disconnected_groups.append(groups[connected_idx])   # all the group numbers that a same node includes
                
                # if there are no same nodes in multi groups, then marked connected nodes as a new group
                if disconnected_groups == []:
                    for connected_idx in connected_nodes:
                        groups[connected_idx] = group_number
                    group_number += 1

                # if there are same nodes in multi groups, then concatenate groups
                else:
                    for idx_ in range(len(groups)):
                        if groups[idx_] in disconnected_groups or idx_ in connected_nodes:
                            groups[idx_] = min(disconnected_groups)
...
class GraphSearch:
    ...
    def search(self, start_idx, goal_idx):
        ...
        while len(unvisited_set) > 0:
            ...
            node_idx = unvisited_set[self.get_minimum_cost_node(unvisited_set)]
            ...
            for neighbour_idx in range(len(self.graph_.nodes_[node_idx].neighbours)):
                ...
                if neighbour.idx in visited_set:
                    ...
                    cost = self.graph_.nodes_[node_idx].cost + neighbour_cost + neighbour.distance_to(self.graph_.nodes_[goal_idx])*self.heuristic_weight_
    ...
    def generate_path(self, goal_idx):
        ...
        # generate the path by traversing from goal node to the start node
        while True:
            parent_idx = current.parent_node
            if parent_idx == None:
                break
            current = self.graph_.nodes_[parent_idx]
            path.append(current)
        
        # reversing the path to get it from start node to goal node
        path = path[::-1]
    ...
    def find_connected_nodes(self, start_idx):
        ...
        while len(unvisited_set) > 0:
            ...
            unvisited_set.remove(node_idx)
            visited_set.append(node_idx)
            for idx in range(len(self.graph_.nodes_[node_idx].neighbours)):
                neighbour_node = self.graph_.nodes_[node_idx].neighbours[idx]
                neighbour_idx = neighbour_node.idx
                if not neighbour_idx in visited_set:
                    if not neighbour_idx in unvisited_set:
                        unvisited_set.append(neighbour_idx)    
...
class PathSmoother():
    ...
    def smooth_path(self, path_nodes):
        ...
        epsilon = 0.001 # tolerance value for error

        # smoothing
        while True:
            error = 0
            temp_path = path_smooth.copy()  # temporary path smooth array

            for i in range(1, len(path_smooth)-1):
                # x and y of waypoints after smoothing iteration
                new_s_x = path_smooth[i].x - (alpha+2*beta)*path_smooth[i].x + alpha*path_nodes[i].x + beta*path_smooth[i-1].x + beta*path_smooth[i+1].x
                new_s_y = path_smooth[i].y - (alpha+2*beta)*path_smooth[i].y + alpha*path_nodes[i].y + beta*path_smooth[i-1].y + beta*path_smooth[i+1].y
                
                # calculating the error
                error += ((new_s_x-path_smooth[i].x)**2 + (new_s_y-path_smooth[i].y)**2)

                # assigning smoothed values
                temp_path[i].x = new_s_x
                temp_path[i].y = new_s_y
            
            # check for wall collisions in the new path
            occluded = False
            for i in range(len(path_smooth)-1):
                if is_occluded(self.graph_.map_.image_, [temp_path[i].x, temp_path[i].y], [temp_path[i+1].x, temp_path[i+1].y]):
                    occluded = True
                    break
            
            # If there are wall collisions, then break the loop
            if occluded:
                break     
            # else, get the smoothed path       
            else:
                path_smooth = temp_path
     
            # break when error is less than tolerance value
            if error < epsilon:
                break
